...
#     Fixes 'There is no current event loop in thread ...' in threaded servers.
#     """
#     try:
...
#     """
#     Convert a PDF into per-page Markdown/text chunks and wrap as Documents.
#     pymupdf4llm.to_markdown(page_chunks=True) returns a list[dict] (one per page).
#     """
#     data = pymupdf4llm.to_markdown(pdf_path, page_chunks=True)
#     docs: List[Document] = []
#     for i, page in enumerate(data):
#         # Some versions expose 'text', others include 'markdown'
...
#             return jsonify({"error": "Add your Gemini API key in main.py or set GOOGLE_API_KEY env var."})
#         try:
...
#             "If the answer is not in the context, say \"I can’t find that in the provided PDF context.\""
#         )
...
#     # OPTIONAL: run single-threaded to sidestep thread loop issues during dev
#     # app.run(port=int(os.environ.get("PORT", 8050)), threaded=False)
